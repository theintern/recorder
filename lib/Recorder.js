(typeof define === 'function' && define.amd ? define : /* istanbul ignore next */ function (factory) {
	...
	function KeyReference(name, value) {
    	this.name = name;
    	this.value = value;
    	this.toString = function(){
    		return 'keys.'+this.name.toUpperCase();
    	}
	}
	...
	function createCommandText(method, args, indent) {
		...
		if (args && args.length) {
			args.forEach(function (arg, index) {
				...
				if( arg instanceof KeyReference){
	...
	var getSeleniumKey = (function () {
		...
		return function (key, keyLocation) {
	...
	Recorder.prototype = {
		...
		_recordKey: function (event) {
			...
			var key = event.key;
			var seleniumKey = getSeleniumKey(key, event.location);

			if(seleniumKey !== false){
				if(isModifierKey(key)){
					key = specialKey;
				} else {
					//If this key is a selenium key and it is not a modifier key, send a KeyReference Object. This o
					this._record('pressKeys', [ new KeyReference(key, seleniumKey) ]);
					return;
				}
			}
			...
			if (event.key.slice(0, 2) === 'U+') {
				var char = String.fromCharCode(Number('0x' + event.key.slice(2)));
				key = event.shiftKey ? char.toUpperCase() : char.toLowerCase();
			}
			...
			if (lastCommand && lastCommand.method === 'pressKeys') {
				if(args[0] instanceof KeyReference){
